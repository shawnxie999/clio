...
/**
 * @brief Pull CFT data from TX via ETLService.
 *
 * @param txMeta Transaction metadata
 * @param sttx The transaction
 * @return CFTsData vector
 */
std::vector<CFTsData>
getCFTDataFromTx(ripple::TxMeta const& txMeta, ripple::STTx const& sttx);

/**
 * @brief Pull CFT data from ledger object via loadInitialLedger.
 *
 * @param seq The ledger sequence to pull for
 * @param key The owner key
 * @param blob Object data as blob
 * @return The CFT data as a vector
 */
std::vector<CFTsData>
